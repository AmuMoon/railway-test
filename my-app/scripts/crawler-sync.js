...
async function fetchPlayerData(accountId) {
  try {
    ...
    console.error(`Failed to fetch player ${accountId}:`, error.message);
    return null;
  }
}
...
async function main() {
  ...
  console.log(`Starting crawl for ${PLAYERS.length} players...`);

  for (const player of PLAYERS) {
    try {
      console.log(`Fetching ${player.name} (${player.accountId})...`);
      ...
      if (data) {
        ...
        console.log(`✓ ${player.name} - ${data.personaname || 'Unknown'}`);
      } else {
  ...
  console.log(`Success: ${players.length}/${PLAYERS.length}`);

  // 推送到 Railway
  const railwayUrl = process.env.RAILWAY_URL || "https://railway-test-production-c4df.up.railway.app";
  const apiKey = process.env.CRAWL_API_KEY || "dev-key";

  console.log(`\nSyncing to Railway: ${railwayUrl}`);
  
  try {
    const response = await fetch(`${railwayUrl}/api/sync`, {
      method: "POST",
      headers: {
